...
class Client:
    def __init__(self, login, password):
        ...
        if not os.path.exists("auth.session"):
            if session.post('https://passport.yandex.ru/passport?mode=auth',
                            data={'login': login, 'passwd': password}).url != 'https://passport.yandex.ru/profile':
    ...
    def get_courses(self):
        profile_json = self.session.get('https://lyceum.yandex.ru/api/profile',
                                        params={'onlyActiveCourses': True,
                                                'withCoursesSummary': True,
                                                'withExpelled': True}).json()
        return [
            {'title': course['title'],
             'course_id': course['id'],
             'group_id': course['group']['id']} for course in profile_json['coursesSummary']['student']
        ]

    def get_course(self, course_id, group_id):
        return self.session.get(
            f"https://lyceum.yandex.ru/api/student/lessons?courseId={course_id}&groupId={group_id}").json()

    def get_lessons(self, course_id, group_id):
        return self.session.get('https://lyceum.yandex.ru/api/student/lessons',
                                params={'groupId': group_id, 'courseId': course_id}).json()

    def get_material(self, lesson_id, group_id, material_id):
        return self.session.get(f"https://lyceum.yandex.ru/api/student/materials/{material_id}",
                                params={'groupId': group_id, 'lessonId': lesson_id}).json()
